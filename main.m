...
% Point force  
A = 10e6; % Amplitude 
ix = round(mx * 0.05); % Coordinates for point force 
iy = round(my*0.95);
...
for i = 1:length(freq_vec)
    ...
    % Amp in room
    room_amp_matrix_s = sqrt(s1(room_y_boundaries_exl, room_x_boundaries_exl).^2 + s2(room_y_boundaries_exl, room_x_boundaries_exl).^2);
    room_amp_matrix_r = sqrt(r1(room_y_boundaries_exl, room_x_boundaries_exl).^2 + r2(room_y_boundaries_exl, room_x_boundaries_exl).^2);
...
fig2 = figure;
plot(freq_vec, max_room_amp, 'LineWidth', 1.5);
grid on;
xlabel('Frequency [Hz]');
